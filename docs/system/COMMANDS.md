...
- 例 `!8`\
  ...
  コマンドを短縮できます。\
  `w=`は`work=`と同じ意味で、`m=`は`min=`と同じ意味です。
  ...
  ### できるだけ若い番号の席に入室する
  `!0`  
  0番席は存在しませんが、`!0`と書くことで、空いている席のうち最も若い番号の席に入室できます。
...
- 入室してからの経過時間（分）
- 自動退室までの残り時間（分）
- （作業中であれば）作業再開までの残り時間（分）


## 休憩する
### `!break`もしくは`!chill`もしくは`!rest`
休憩モードに切り替えます。入室中のみ使えます。\
休憩モードについて詳細は[こちら](https://youtube-study-space.notion.site/da1f6ff5cc6b4774944f9012798df4e4)

  ### オプション `min=休憩時間` `m=休憩時間`
  休憩時間（分）を指定できます。\
  設定可能な値は`3`～`30`の整数です。

  - 例 `!break`\
    休憩モードに切り替える。休憩時間はデフォルトの5分。
  - 例 `!break m=20`\
    休憩モードに切り替える。休憩時間は20分。
